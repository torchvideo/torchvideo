...
def set_video_backend(backend: str) -> None:
    ...
    if backend == 'pyav':
        try:
            ...
            warnings.warn("PyAV not installed, defaulting to {}".format(_default_backend), RuntimeWarning)
            backend = _default_backend
    if backend == 'nvvl':
        try:
            import nvvl
        except ImportError:
            warnings.warn("nvvl not installed, defaulting to {}".format(_default_backend), RuntimeWarning)
            backend = _default_backend
