...
_VIDEO_FILE_EXTENSIONS = {'mp4', 'webm', 'avi', '3gp',
                         'wmv', 'mpg', 'mpeg', 'mov',
                         'mkv'}
...
class VideoDataset(torch.utils.data.Dataset, ABC):
    ...
    def __getitem__(self, index) -> Union[torch.Tensor, Tuple[torch.Tensor, Label]]:  # pragma: no cover
...
class ImageFolderVideoDataset(VideoDataset):
    ...
    def __init__(
        ...
        self.video_lengths = [len(list(video_dir.iterdir())) for video_dir in self.video_dirs]
    ...
    def _load_frames(self, frames_idx: Union[slice, List[slice], List[int]], video_folder: Path)\
            -> np.ndarray:
        frame_numbers = frame_idx_to_list(frames_idx)
        filepaths = [video_folder / self.filename_template.format(index + 1) for index in frame_numbers]
...
class VideoFolderDataset(VideoDataset):
    ...
    def __init__(
        ...
        super(VideoFolderDataset, self).__init__(root_path, label_set=label_set, sampler=sampler)
        self.video_paths = sorted([child for child in root_path.iterdir() if _is_video_file(child)])
        self.video_lengths = []
    ...
    def _load_frames(self, frame_idx: Union[slice, List[slice], List[int]], video_file: Path) -> torch.Tensor:
...
class GulpVideoDataset(VideoDataset):
    def __init__(self, root_path: Union[str, Path],
                 label_set: LabelSet = None,
                 sampler: FrameSampler = _default_sampler()):
        super(GulpVideoDataset, self).__init__(root_path, label_set=label_set, sampler=sampler)
    ...
    def __getitem__(self, index) -> Union[torch.Tensor, Tuple[torch.Tensor, Label]]:
        ...
        if isinstance(frame_idx, slice):
            ...
            if isinstance(frame_idx[0], slice):
                return torch.Tensor(np.array([self.gulp_dir[id_, slice_][0] for slice_ in frame_idx]))
            elif isinstance(frame_idx[0], int):
                return torch.Tensor(np.array([self.gulp_dir[id_, slice(index, index + 1)][0]
